...
const AddWishList = ({ data }) => {
    ...
    //             dispatch(handleRemoveWishlist(data?._id)); // 🏷 Remove from Redux
    ...
    //                 dispatch(handleAddWishlist({ _id: data?._id, productId: data })); // ✅ Add full data
    ...
    return (
        <button onClick={handleToggleWishlist} className="flex items-center justify-center" disabled={loading}>
            {isAvailableWishlist ? (
                <FaHeart size={26} className="text-red-500 transition-all duration-300" />
            ) : (
                <FaRegHeart size={26} className="text-gray-500 transition-all duration-300" />
            )}
        </button>
    );
};
